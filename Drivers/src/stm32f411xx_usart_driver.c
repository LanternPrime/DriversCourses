...
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint32_t BaudRate)
{
    ...
    {
        // OVER8 = 1 , over sampling by 8
        F_part = (((F_part * OVERSAMPLING_8) + 50) / 100) & ((uint8_t)0x07);
    }
    else
    {
        // over sampling by 16
        F_part = (((F_part * OVERSAMPLING_16) + 50) / 100) & ((uint8_t)0x0F);
    }
...
void USART_IRQHandling(USART_Handle_t *pUSARTHandle)
{
    ...
    uint32_t flag_set, interrupt_enabled, temp3;
    uint16_t *pdata;

    /*************************Check for TC flag ********************************************/

    // Implement the code to check the state of TC bit in the SR
    flag_set = pUSARTHandle->pUSARTx->SR & (1 << USART_SR_TC);

    // Implement the code to check the state of TCEIE bit
    interrupt_enabled = pUSARTHandle->pUSARTx->CR1 & (1 << USART_CR1_TCIE);
    ...
    flag_set = pUSARTHandle->pUSARTx->SR & (1 << USART_SR_TXE);

    // Implement the code to check the state of TXEIE bit in CR1
    interrupt_enabled = pUSARTHandle->pUSARTx->CR1 & (1 << USART_CR1_TXEIE);
    ...
    flag_set = pUSARTHandle->pUSARTx->SR & (1 << USART_SR_RXNE);
    interrupt_enabled = pUSARTHandle->pUSARTx->CR1 & (1 << USART_CR1_RXNEIE);
    ...
    flag_set = pUSARTHandle->pUSARTx->SR & (1 << USART_SR_CTS);

    // Implement the code to check the state of CTSE bit in CR1
    interrupt_enabled = pUSARTHandle->pUSARTx->CR3 & (1 << USART_CR3_CTSE);

    // Implement the code to check the state of CTSIE bit in CR3 (This bit is not available for UART4 & UART5.)
    temp3 = pUSARTHandle->pUSARTx->CR3 & (1 << USART_CR3_CTSIE);

    if (flag_set && interrupt_enabled && temp3)
    {
    ...
    flag_set = pUSARTHandle->pUSARTx->SR & (1 << USART_SR_IDLE);

    // Implement the code to check the state of IDLEIE bit in CR1
    interrupt_enabled = pUSARTHandle->pUSARTx->CR1 & (1 << USART_CR1_IDLEIE);
    ...
    flag_set = pUSARTHandle->pUSARTx->SR & USART_SR_ORE;

    // Implement the code to check the status of RXNEIE  bit in the CR1
    interrupt_enabled = pUSARTHandle->pUSARTx->CR1 & USART_CR1_RXNEIE;
    ...
    interrupt_enabled = pUSARTHandle->pUSARTx->CR3 & (1 << USART_CR3_EIE);

    if (interrupt_enabled)
    {
        flag_set = pUSARTHandle->pUSARTx->SR;
        if (flag_set & (1 << USART_SR_FE))
        {
            /*
