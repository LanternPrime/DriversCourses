...
int main(void)
{
    ...
    while (1)
    {
        ...
        while (I2C_MasterSendDataIT(&I2C1Handler, &cmdCode, 1, SLAVE_ADDR, I2C_ENABLE_SR) != I2C_READY);

        while (I2C_MasterReceiveDataIT(&I2C1Handler, &len, 1, SLAVE_ADDR, I2C_ENABLE_SR) != I2C_READY);
        ...
        while (I2C_MasterSendDataIT(&I2C1Handler, &cmdCode, 1, SLAVE_ADDR, I2C_ENABLE_SR) != I2C_READY);
        printf("len> %d\n", len);
        while (I2C_MasterReceiveDataIT(&I2C1Handler, rx_data, len, SLAVE_ADDR, I2C_DISABLE_SR) != I2C_READY);

        rxComplete = RESET;
        while (rxComplete != SET);
        rx_data[len + 1] = '\0';

        printf("Data> %s", rx_data);
